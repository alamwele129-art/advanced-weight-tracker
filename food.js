...
import {
    ...
    Modal,
    I18nManager,
    // useColorScheme, // No longer needed here directly for main theme decision
    Alert,
    Platform,
    Image,
    FlatList,
...
import {
    MEAL_CATEGORIES,
    MEAL_CATEGORIES_ORDER,
    DEFAULT_MEAL_TARGETS,
    MEAL_IMAGES,
    PREDEFINED_FOODS
} from './FoodData';

// AsyncStorage Keys for data, not for app settings (language/dark mode)
...
// FoodLogScreen receives language and darkMode as props from App.js
const FoodLogScreen = ({ language, darkMode }) => {
    // Use props directly for language and darkMode
    const styles = darkMode ? darkStyles : lightStyles;

    const translations = useMemo(() => ({ // Encapsulate translations to recompute only if language changes
        ar: {
            ...
            loadingData: "جاري تحميل البيانات..." // Added
        },
        en: { 
    ...
    const [isScreenDataLoaded, setIsScreenDataLoaded] = useState(false); // For initial data load
    ...
    const appContentStartDate = useMemo(() => new Date(2025, 4, 19), []); // Example: May 19, 2025
    ...
    // Removed language, darkMode, isInitialized states from here
    // App.js manages language and darkMode and passes them as props.
    // isInitialized (for app settings) is effectively managed by App.js's loading sequence.
    ...
    // Removed systemColorScheme as darkMode is a prop
    // Removed initializeAppSettings useEffect
    ...
    const loadDataForDate = useCallback(async (dateToLoad) => {
        setIsScreenDataLoaded(false); // Set loading true when starting
        ...
        let macrosWereInitialized = false;
        const currentDataKey = getMealsDataKeyForDate(dateToLoad);
        let savedDataJSON;
    
        try {
            savedDataJSON = await AsyncStorage.getItem(currentDataKey);
            if (savedDataJSON) {
                const parsedData = JSON.parse(savedDataJSON);
                if (parsedData.mealsData) {
                    MEAL_CATEGORIES_ORDER.forEach(catKey => {
                        if (!parsedData.mealsData[catKey]) {
                            parsedData.mealsData[catKey] = initializeEmptyMealsData()[catKey];
                        } else {
                            parsedData.mealsData[catKey].image = MEAL_IMAGES[catKey] || MEAL_IMAGES.breakfast;
                            if (typeof parsedData.mealsData[catKey].targetCalories !== 'number') {
                                parsedData.mealsData[catKey].targetCalories = initializeEmptyMealsData()[catKey].targetCalories;
                            }
                        }
                    });
                    loadedMealsData = parsedData.mealsData;
                }
                currentDailyTarget = parsedData.dailyTargetCalories !== undefined ? parsedData.dailyTargetCalories : initialDailyCalories;
                if (currentDailyTarget > MAX_DAILY_TARGET_CALORIES) currentDailyTarget = MAX_DAILY_TARGET_CALORIES;
                
                if (parsedData.targetProtein !== undefined && parsedData.targetFat !== undefined && parsedData.targetCarbs !== undefined) {
                    currentTargetP = parsedData.targetProtein;
                    currentTargetF = parsedData.targetFat;
                    currentTargetC = parsedData.targetCarbs;
                } else {
                ...
                macrosWereInitialized = parsedData.macrosInitializedFromImage || false;
            } else { // No saved data for this date
                ...
                const isAppContentStartDateDay = dateToLoad.getFullYear() === appContentStartDate.getFullYear() &&
                                               dateToLoad.getMonth() === appContentStartDate.getMonth() &&
                                               dateToLoad.getDate() === appContentStartDate.getDate();
                const initialSetupKey = getInitialSetupDoneKeyForDate(dateToLoad);
                const setupDone = await AsyncStorage.getItem(initialSetupKey);

                if (isAppContentStartDateDay && !setupDone) {
                    const initialHotdogItem = {
                        id: 'initial1',
                        name: language === 'ar' ? 'هوت دوج خاص' : 'Special Hotdog', // Uses language prop
                        calories: 200, protein: 10, fat: 15, carbs: 5
                    };
                    loadedMealsData = initializeEmptyMealsData(); 
                    loadedMealsData.breakfast.items = [initialHotdogItem];
                    currentDailyTarget = 2213; 
                    if (currentDailyTarget > MAX_DAILY_TARGET_CALORIES) currentDailyTarget = MAX_DAILY_TARGET_CALORIES;
                    const hotdogDayMacros = calculateMacroTargetsFromCalories(currentDailyTarget, DEFAULT_MACRO_PERCENTAGES);
                    currentTargetP = hotdogDayMacros.proteinG;
                    currentTargetF = hotdogDayMacros.fatG;
                    currentTargetC = hotdogDayMacros.carbsG;
                    macrosWereInitialized = true; 
                    await AsyncStorage.setItem(initialSetupKey, 'true');
                }
            }
        } catch (error) {
            ...
            currentDailyTarget = initialDailyCalories;
            if (currentDailyTarget > MAX_DAILY_TARGET_CALORIES) currentDailyTarget = MAX_DAILY_TARGET_CALORIES;
            const errorFallbackMacros = calculateMacroTargetsFromCalories(currentDailyTarget, DEFAULT_MACRO_PERCENTAGES);
            currentTargetP = errorFallbackMacros.proteinG;
            currentTargetF = errorFallbackMacros.fatG;
            currentTargetC = errorFallbackMacros.carbsG;
        }
        ...
        if (!savedDataJSON && (macrosWereInitialized || (dateToLoad.getFullYear() === appContentStartDate.getFullYear() && dateToLoad.getMonth() === appContentStartDate.getMonth() && dateToLoad.getDate() === appContentStartDate.getDate() && !(await AsyncStorage.getItem(getInitialSetupDoneKeyForDate(dateToLoad)))))) {
             await saveMealsDataInternal(loadedMealsData, currentDailyTarget, currentTargetP, currentTargetF, currentTargetC, macrosWereInitialized, dateToLoad);
        }
        setIsScreenDataLoaded(true); // Set loading false after completion
    }, [appContentStartDate, language, saveMealsDataInternal, getMealsDataKeyForDate, getInitialSetupDoneKeyForDate, initialDailyCalories]); 


    useEffect(() => {
        // language prop is now a dependency for loadDataForDate
        loadDataForDate(selectedDate);
    }, [selectedDate, language, loadDataForDate]); // Added language dependency


    useEffect(() => {
        if (!isScreenDataLoaded || !mealsData) return; // Wait for data to be loaded
        ...
        setTotalProtein(currentP);
        setTotalFat(currentF);
        setTotalCarbs(currentC);
        setMealCount(itemCount);
    ...
    useEffect(() => {
        ...
        const DebouncedSave = setTimeout(async () => {
            const currentDataKey = getMealsDataKeyForDate(selectedDate);
            const savedDataRaw = await AsyncStorage.getItem(currentDataKey);
            let savedData = {};
            try {
                 savedData = savedDataRaw ? JSON.parse(savedDataRaw) : {};
            } catch (e) {
                console.error("Error parsing saved data on save check:", e);
            }

            const { currentTotalCals, itemCount } = calculateTotalsFromMealsData(mealsData);

            const hasDataChanged =
                JSON.stringify(mealsData) !== JSON.stringify(savedData.mealsData) ||
                dailyTargetCalories !== savedData.dailyTargetCalories ||
                targetProtein !== savedData.targetProtein || 
                targetFat !== savedData.targetFat ||         
                targetCarbs !== savedData.targetCarbs ||     
                itemCount !== savedData.mealCount ||
                currentTotalCals !== savedData.totalCalories;

            if (hasDataChanged) {
                const macrosWereSetFromImage = savedData.macrosInitializedFromImage || false;
                const newMacrosInitialized = itemCount > 0 ? macrosWereSetFromImage : false;
    ...
    const addMeal = useCallback(() => {
        ...
        if (remainingCalories <= 0 && caloriesNum > 0) { 
            setErrorMessage(translation.calorieLimitReached);
            return;
        }
        ...
        if (mealName.trim() && !isNaN(caloriesNum) && caloriesNum >= 0 && selectedMealCategory) {
            ...
            setMealsData(prev => {
                ...
                if (!updated[selectedMealCategory]) {
                    updated[selectedMealCategory] = initializeEmptyMealsData()[selectedMealCategory];
                }
    ...
    const handleOpenTargetEditModal = () => {
        if (!isTodaySelected) {
            Alert.alert(translation.editPastDayTitle, translation.editPastDayMessageTarget);
            return;
        }
        setEditableTargetCalories(String(dailyTargetCalories));
        setTargetEditError('');
    ...
    const handleSaveTargetCalories = () => {
        ...
        if (isNaN(newTargetCalories) || newTargetCalories <= 0) {
            setTargetEditError(translation.invalidTargetError);
            return;
        }
        if (newTargetCalories > MAX_DAILY_TARGET_CALORIES) {
            setTargetEditError(`${translation.invalidTargetErrorMaxPrefix}${translation.invalidTargetErrorMaxSuffix}${MAX_DAILY_TARGET_CALORIES.toLocaleString(language === 'ar' ? 'ar-EG' : 'en-US')}`);
            return;
        }

        const newMacroTargets = calculateMacroTargetsFromCalories(newTargetCalories, DEFAULT_MACRO_PERCENTAGES);
        setTargetProtein(newMacroTargets.proteinG);
        setTargetFat(newMacroTargets.fatG);
        setTargetCarbs(newMacroTargets.carbsG);

        const newMealsData = JSON.parse(JSON.stringify(mealsData));
        let currentTotalOfIndividualMealTargets = 0;
        MEAL_CATEGORIES_ORDER.forEach(categoryKey => {
            if (mealsData[categoryKey] && typeof mealsData[categoryKey].targetCalories === 'number') {
                currentTotalOfIndividualMealTargets += mealsData[categoryKey].targetCalories;
            }
        });

        if (currentTotalOfIndividualMealTargets > 0) {
            MEAL_CATEGORIES_ORDER.forEach(categoryKey => {
                if (newMealsData[categoryKey]) {
                    const currentMealTarget = (mealsData[categoryKey] && typeof mealsData[categoryKey].targetCalories === 'number') 
                                              ? mealsData[categoryKey].targetCalories 
                                              : 0;
                    const proportion = currentMealTarget / currentTotalOfIndividualMealTargets;
                    newMealsData[categoryKey].targetCalories = Math.round(newTargetCalories * proportion);
                }
            });
        } else {
            let defaultTotalTargets = 0;
            MEAL_CATEGORIES_ORDER.forEach(key => { defaultTotalTargets += (DEFAULT_MEAL_TARGETS[key] || 0); });
            if (defaultTotalTargets > 0) {
                MEAL_CATEGORIES_ORDER.forEach(categoryKey => {
                    if (newMealsData[categoryKey]) {
                        const defaultMealTarget = DEFAULT_MEAL_TARGETS[categoryKey] || 0;
                        const proportion = defaultMealTarget / defaultTotalTargets;
                        newMealsData[categoryKey].targetCalories = Math.round(newTargetCalories * proportion);
                    }
                });
            } else {
                const numCategories = MEAL_CATEGORIES_ORDER.length;
                if (numCategories > 0) {
                    const equalShare = Math.round(newTargetCalories / numCategories);
                    let sumAssigned = 0;
                    MEAL_CATEGORIES_ORDER.forEach((categoryKey, index) => {
                        if (newMealsData[categoryKey]) {
                            if (index < numCategories - 1) {
                                newMealsData[categoryKey].targetCalories = equalShare;
                                sumAssigned += equalShare;
                            } else {
                                newMealsData[categoryKey].targetCalories = newTargetCalories - sumAssigned;
                            }
                        }
                    });
                }
        ...
        let finalSumOfMealTargets = 0;
        MEAL_CATEGORIES_ORDER.forEach(categoryKey => {
            if (newMealsData[categoryKey] && typeof newMealsData[categoryKey].targetCalories === 'number') {
                finalSumOfMealTargets += newMealsData[categoryKey].targetCalories;
            }
        });
        const difference = newTargetCalories - finalSumOfMealTargets;
        if (difference !== 0 && MEAL_CATEGORIES_ORDER.length > 0) {
            const lastCategoryKey = MEAL_CATEGORIES_ORDER[MEAL_CATEGORIES_ORDER.length -1];
            if (newMealsData[lastCategoryKey] && typeof newMealsData[lastCategoryKey].targetCalories === 'number') {
                newMealsData[lastCategoryKey].targetCalories += difference;
                if (newMealsData[lastCategoryKey].targetCalories < 0) {
                    newMealsData[lastCategoryKey].targetCalories = 0;
                }
            }
        }
        ...
        setTargetEditModalVisible(false);
        setTargetEditError('');
        Alert.alert(translation.targetUpdated);
    };

    const resetMeals = useCallback(() => {
        if (!isTodaySelected) {
            Alert.alert(translation.editPastDayTitle, translation.editPastDayMessageReset);
            return;
        }
        ...
        setTargetProtein(resetMacros.proteinG);
        setTargetFat(resetMacros.fatG);
        setTargetCarbs(resetMacros.carbsG);
        setDailyTargetCalories(initialDailyCalories);
    ...
    useEffect(() => {
        setCurrentSuggestionsScrollX(0);
        if (suggestionsScrollRef.current) {
            suggestionsScrollRef.current.scrollTo({ x: 0, animated: false });
        }
        setShowPrevSuggestionArrow(false);
        setShowNextSuggestionArrow(currentSuggestions.length > MAX_SUGGESTIONS_BEFORE_ARROW_LOGIC);
    }, [selectedMealCategory, currentSuggestions.length]);
    ...
    if (!isScreenDataLoaded) { 
        return ( 
            <View style={[styles.wrapper, { justifyContent: 'center', alignItems: 'center' }]}> 
                ...
                <Text style={{ color: darkMode ? styles.title.color : styles.title.color, marginTop:10 }}>
                    {translation.loadingData}
                </Text> 
            </View> 
        ); 
    }

    const suggestionScrollViewPadding = {
        paddingLeft: showPrevSuggestionArrow ? SUGGESTION_ARROW_WIDTH : 0,
        paddingRight: showNextSuggestionArrow ? SUGGESTION_ARROW_WIDTH : 0,
    };

    return (
        <View style={styles.wrapper}>
            {/* Added key to ScrollView to help with re-renders on language/theme change if necessary,
                though individual components should update with props. Consider removing if performance issues arise. */}
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContentContainer} keyboardShouldPersistTaps="handled" key={`${language}-${darkMode}-${selectedDate.toDateString()}`}>
                <View style={styles.innerContainer}>
                    ...
                    {/* DateNavigator, CalorieProgressDisplay receive language and darkMode props */}
                    ...
                    <View style={styles.targetDisplayContainer}>
                        ...
                        <TouchableOpacity 
                            onPress={handleOpenTargetEditModal} 
                            style={[styles.editTargetButton, !isTodaySelected && styles.disabledButton]}
                    ...
                    <View style={styles.buttonsContainer}>
                        <TouchableOpacity 
                            style={[
                                styles.addMealBtn, 
                                (!canAddMoreCalories || !isTodaySelected) && styles.disabledButton
                            ]} 
                            onPress={() => { 
                                if (!isTodaySelected) {
                                    Alert.alert(translation.editPastDayTitle, translation.editPastDayMessage);
                                ...
                                if (!canAddMoreCalories) { return; } 
                                setModalVisible(true); 
                            }} 
                            disabled={!canAddMoreCalories || !isTodaySelected}
                        >
                            <Text style={styles.buttonText}>{translation.addMealBtn}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                            style={[styles.resetBtn, !isTodaySelected && styles.disabledButton]}
                            onPress={resetMeals}
                            disabled={!isTodaySelected}
                        >
                    ...
                    <View style={styles.mealCategoriesContainer}>{MEAL_CATEGORIES_ORDER.map(categoryKey => { 
                        ...
                        <TouchableOpacity 
                            ...
                            onPress={() => { 
                                ...
                                if (!canAddMoreCalories && categoryData.items.length === 0) { return; } 
                                setSelectedMealCategory(categoryKey); 
                                setModalVisible(true); 
                            }} 
                            disabled={!isTodaySelected && categoryData.items.length === 0} 
                        >
            ...
            <Modal visible={isModalVisible} transparent={true} animationType="slide" onRequestClose={handleCloseAddMealModal}>
                <View style={styles.modalOverlay}><View style={styles.modalContent}><ScrollView keyboardShouldPersistTaps="handled">
                    ...
                    <View style={[styles.pickerContainer, !isTodaySelected && styles.disabledPickerContainer]}>
                        <Picker 
                            selectedValue={selectedMealCategory} 
                            onValueChange={(itemValue) => { if(isTodaySelected) setSelectedMealCategory(itemValue);}} 
                            style={styles.picker} 
                            itemStyle={styles.pickerItem} // Ensure pickerItem styles are applied if needed
                            dropdownIconColor={darkMode ? '#FFFFFF' : '#555'} // Added for consistency
                    ...
                    <TouchableOpacity 
                        style={[styles.addButton, ((!canAddMoreCalories && mealCalories !== '' && parseInt(mealCalories) > 0) || !isTodaySelected) && styles.disabledButton]} 
                        onPress={addMeal} 
                        disabled={(!canAddMoreCalories && mealCalories !== '' && (parseInt(mealCalories) || 0) > 0) || !isTodaySelected}
                    >
            ...
            <Modal
                visible={isTargetEditModalVisible}
                transparent={true}
                animationType="slide"
                onRequestClose={() => {
                    setTargetEditModalVisible(false);
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput
                            style={[styles.input, !isTodaySelected && styles.disabledInput]}
                            placeholder={translation.enterNewTarget}
                            keyboardType="numeric"
                            value={editableTargetCalories}
                            onChangeText={setEditableTargetCalories}
                        ...
                        <View style={baseStyles.modalButtonsContainer}>
                            <TouchableOpacity
                                style={[ baseStyles.modalButton, darkMode ? darkStyles.cancelButton : lightStyles.cancelButton, { marginRight: Platform.OS === 'ios' ? 5 : (I18nManager.isRTL ? 0 : 5) , marginLeft: Platform.OS === 'ios' ? 0 : (I18nManager.isRTL ? 5 : 0)} ]}
                                onPress={() => { setTargetEditModalVisible(false); setTargetEditError(''); }}>
                                <Text style={[ baseStyles.modalButtonText, darkMode ? darkStyles.modalButtonText : lightStyles.modalButtonText ]}>
                            ...
                            <TouchableOpacity
                                style={[ 
                                    ...
                                    darkMode ? darkStyles.saveButton : lightStyles.saveButton, 
                                    !isTodaySelected && styles.disabledButton,
                                    { marginLeft: Platform.OS === 'ios' ? 5 : (I18nManager.isRTL ? 0 : 5) , marginRight: Platform.OS === 'ios' ? 0 : (I18nManager.isRTL ? 5 : 0)} 
                                ]}
                                onPress={handleSaveTargetCalories}
                                disabled={!isTodaySelected}
                                >
                                <Text style={[ baseStyles.modalButtonText, darkMode ? darkStyles.saveModalButtonText : lightStyles.saveModalButtonText ]}>
                                    {translation.saveTargetButton}
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
...
// Styles (baseStyles, lightStyles, darkStyles) remain largely the same.
// Make sure pickerItem in lightStyles and darkStyles has appropriate color for text.
const baseStyles = {
    ...
    pickerItem: Platform.OS === 'android' ? {
        // Android item style can be tricky, color might be inherited or need specific handling
    } : {
        // iOS item style
    },
    ...
    disabledInput: { /* سيتم تعريفه في lightStyles و darkStyles */ },
...
const lightStyles = StyleSheet.create({
    ...
    ...baseStyles,
    pickerItem: { // Added for light mode picker item text color
        ...baseStyles.pickerItem,
        color: '#333333', // Default text color for picker items in light mode
        // backgroundColor for Android might be needed if the picker background doesn't show through
        // backgroundColor: Platform.OS === 'android' ? '#f8f8f8' : undefined, 
    },
    ...
    suggestionScrollArrowContainer: { 
        ...baseStyles.suggestionScrollArrowContainer,
        backgroundColor: 'rgba(230, 230, 230, 0.85)', 
    },
    suggestionScrollArrowText: { 
        ...baseStyles.suggestionScrollArrowText,
        color: '#555555', 
    },
...
const darkStyles = StyleSheet.create({
    ...
    pickerItem: { // Added for dark mode picker item text color
        ...baseStyles.pickerItem,
        color: '#e0e0e0', // Default text color for picker items in dark mode
        // backgroundColor: Platform.OS === 'android' ? '#3b3b3b' : undefined,
    },
