...
import {
    ...
    Pressable, I18nManager, Alert, AppState, ActivityIndicator, Image, useColorScheme,
    Modal, Animated
} from 'react-native';
...
const StepsScreen = (props) => {
    ...
    useEffect(() => { const loadInitialData = async () => { setIsLoading(true); try { const storedGoal = await AsyncStorage.getItem('@Steps:goal'); let loadedGoal = DEFAULT_GOAL; if (storedGoal) { const parsedGoal = parseInt(storedGoal, 10); if (!isNaN(parsedGoal) && GOAL_OPTIONS.includes(parsedGoal)) { loadedGoal = parsedGoal; } } setGoalSteps(loadedGoal); setTempGoal(loadedGoal); goalStepsRef.current = loadedGoal; const history = await getStoredStepsHistory(); const todayString = getDateString(new Date()); const savedTodaySteps = history[todayString] || 0; setCurrentSteps(savedTodaySteps); setStepsForSelectedDay(savedTodaySteps); setDisplaySteps(Math.min(savedTodaySteps, loadedGoal)); } catch (error) { console.error("Failed to load initial data from storage:", error); setCurrentSteps(0); setStepsForSelectedDay(0); setDisplaySteps(0); } finally { setIsLoading(false); } }; loadInitialData(); }, [getStoredStepsHistory]);
    useEffect(() => { const fetchStepsForDate = async () => { if (isViewingToday) { setStepsForSelectedDay(currentSteps); } else { const history = await getStoredStepsHistory(); const dateString = getDateString(selectedDate); setStepsForSelectedDay(history[dateString] || 0); } }; if (!isLoading) { fetchStepsForDate(); } }, [selectedDate, currentSteps, getStoredStepsHistory, isViewingToday, isLoading]);
    useEffect(() => { const stepsToAnimateTo = Math.min(stepsForSelectedDay, goalSteps); animateDisplaySteps(displaySteps, stepsToAnimateTo); }, [stepsForSelectedDay, goalSteps, animateDisplaySteps]);
    useEffect(() => { if (selectedPeriod === 'day') { const newWeekStart = getStartOfWeek(selectedDate, startOfWeekDay); if (newWeekStart.getTime() !== selectedWeekStart.getTime()) { setSelectedWeekStart(newWeekStart); } } }, [selectedDate, startOfWeekDay, selectedWeekStart, selectedPeriod]);
    ...
    const updateChallengeStatus = useCallback(async () => { const todayString = getDateString(new Date()); if (!todayString) return; try { const [ storedRemainingDaysStr, storedLastParticipationDate, storedChallengeDurationStr ] = await Promise.all([ AsyncStorage.getItem(REMAINING_CHALLENGE_DAYS_KEY), AsyncStorage.getItem(LAST_PARTICIPATION_DATE_KEY), AsyncStorage.getItem(CURRENT_CHALLENGE_DURATION_KEY) ]); let loadedDuration = parseInt(storedChallengeDurationStr, 10) || INITIAL_CHALLENGE_DURATION; if (!CHALLENGE_DURATIONS.includes(loadedDuration)) { loadedDuration = INITIAL_CHALLENGE_DURATION; } let currentRemainingDays = parseInt(storedRemainingDaysStr, 10); if (isNaN(currentRemainingDays)) { currentRemainingDays = loadedDuration; } if (todayString !== storedLastParticipationDate) { if (currentRemainingDays > 0) { currentRemainingDays = currentRemainingDays - 1; } if (currentRemainingDays <= 0) { if (loadedDuration > 0) { const storedMaxStr = await AsyncStorage.getItem(MAX_COMPLETED_CHALLENGE_DAYS_KEY); const currentMax = parseInt(storedMaxStr || '0', 10); if (loadedDuration > currentMax) { await AsyncStorage.multiSet([ [MAX_COMPLETED_CHALLENGE_DAYS_KEY, String(loadedDuration)], [CELEBRATE_TIER_COMPLETION_KEY, String(loadedDuration)] ]); } } const currentDurationIndex = CHALLENGE_DURATIONS.indexOf(loadedDuration); const nextDurationIndex = currentDurationIndex + 1; if (nextDurationIndex < CHALLENGE_DURATIONS.length) { const nextChallengeDuration = CHALLENGE_DURATIONS[nextDurationIndex]; await AsyncStorage.multiSet([ [REMAINING_CHALLENGE_DAYS_KEY, String(nextChallengeDuration)], [CURRENT_CHALLENGE_DURATION_KEY, String(nextChallengeDuration)], [LAST_PARTICIPATION_DATE_KEY, todayString] ]); setCurrentChallengeDuration(nextChallengeDuration); setRemainingDays(nextChallengeDuration); setLastParticipationDate(todayString); } else { await AsyncStorage.multiSet([ [REMAINING_CHALLENGE_DAYS_KEY, '0'], [LAST_PARTICIPATION_DATE_KEY, todayString] ]); setCurrentChallengeDuration(loadedDuration); setRemainingDays(0); setLastParticipationDate(todayString); } } else { await AsyncStorage.multiSet([ [REMAINING_CHALLENGE_DAYS_KEY, String(currentRemainingDays)], [LAST_PARTICIPATION_DATE_KEY, todayString] ]); setCurrentChallengeDuration(loadedDuration); setRemainingDays(currentRemainingDays); setLastParticipationDate(todayString); } } else { setCurrentChallengeDuration(loadedDuration); setRemainingDays(currentRemainingDays); } } catch (error) { console.error("Challenge update fail:", error); } }, []);
    ...
    useEffect(() => { if (selectedPeriod === 'day' || selectedPeriod === 'week') { const fetchWeeklyData = async () => { setIsWeeklyLoading(true); try { const stepsForTodayInView = isCurrentWeekSelected ? currentSteps : 0; const currentDisplayWeekStart = getStartOfWeek(selectedWeekStart, startOfWeekDay); const displayedWeekPromise = getWeekDataFromHistory(currentDisplayWeekStart, stepsForTodayInView, getStoredStepsHistory); const comparisonWeekStartDate = addDays(currentDisplayWeekStart, -7); const comparisonWeekPromise = getWeekDataFromHistory(comparisonWeekStartDate, 0, getStoredStepsHistory); const [displayedData, comparisonData] = await Promise.all([displayedWeekPromise, comparisonWeekPromise]); setActualWeekData(displayedData); setPreviousWeekForComparisonData(comparisonData); if (selectedPeriod === 'day') setDailyChartData(displayedData); } catch (error) { console.error("Weekly data fail:", error); setActualWeekData(Array(7).fill(0)); setPreviousWeekForComparisonData(Array(7).fill(0)); if (selectedPeriod === 'day') setDailyChartData(Array(7).fill(0)); } finally { setIsWeeklyLoading(false); } }; fetchWeeklyData(); } }, [selectedWeekStart, currentSteps, selectedPeriod, getStoredStepsHistory, isCurrentWeekSelected, startOfWeekDay]);
    ...
    // ============================================
    // التعديل 1: رفع القائمة لفوق بطرح 10 بيكسل
    // ============================================
    const openTitleMenu = useCallback(() => {
        if (titleMenuTriggerRef.current) {
            titleMenuTriggerRef.current.measure((fx, fy, w, h, px, py) => {
                const top = py + h - 25;
                setTitleMenuPosition({ top, left: undefined, right: undefined, triggerX: 0, triggerWidth: 0 });
                setIsTitleMenuVisible(true);
            });
        }
    }, []);
...
<View style={currentStyles.dayHeader}>
    ...
    {/* 2. الزرار الأيمن (للأيام السابقة) */}
                        ...
                        {isLoading && isViewingToday ? (
                            ...
                            <View style={currentStyles.mainDisplayArea}>
                                <View style={currentStyles.circle}>
                                    ...
                                    {clampedProgress < 100 && stepsForSelectedDay > 0 && (
...
<Modal visible={isMenuModalVisible} transparent={true} animationType="fade" onRequestClose={closeMenuModal} >
                <Pressable style={currentStyles.menuModalOverlay} onPress={closeMenuModal}>
                    ...
                    {/* --- السهم الصغير --- */}
                    <View style={[ 
                        currentStyles.menuArrow, 
                        { 
                            // التعديل هنا:
                            // جعلنا الرقم -29 بدلاً من -30 أو -18
                            // هذا يضيف 1 بيكسل تداخل لإخفاء أي فراغ
                            top: menuPosition.top ? menuPosition.top - MENU_ARROW_HEIGHT - 29 : 0, 
                            
                            left: I18nManager.isRTL ? 32 : undefined, 
                            right: I18nManager.isRTL ? undefined : 38, 
                        } 
                    ]} />
                    {/* --- المربع الأبيض (القائمة) --- */}
                    <View style={[ 
                        currentStyles.menuModalContent, 
                        { 
             ...
             <Modal visible={isTitleMenuVisible} transparent={true} animationType="fade" onRequestClose={closeTitleMenu} >
                <Pressable style={currentStyles.menuModalOverlay} onPress={closeTitleMenu}>
                    <View style={[ 
                        currentStyles.titleMenuModalContent, 
                        { 
