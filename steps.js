...
const StepsScreen = (props) => {
        ...
        const loadInitialData = async () => {
            ...
            try {
                ...
                if (storedGoal) {
                    ...
                    if (!isNaN(parsedGoal) && GOAL_OPTIONS.includes(parsedGoal)) {
                        loadedGoal = parsedGoal;
                    }
                }
                setGoalSteps(loadedGoal);
                setTempGoal(loadedGoal);
                goalStepsRef.current = loadedGoal;
                const history = await getStoredStepsHistory();
                const todayString = getDateString(new Date());
                const savedTodaySteps = history[todayString] || 0;
                setCurrentSteps(savedTodaySteps);
                setStepsForSelectedDay(savedTodaySteps);
                setDisplaySteps(Math.min(savedTodaySteps, loadedGoal));
            } catch (error) {
                console.error("Failed to load initial data from storage:", error);
                setCurrentSteps(0);
                setStepsForSelectedDay(0);
                setDisplaySteps(0);
            } finally {
                setIsLoading(false);
            }
    ...
    useEffect(() => {
        const fetchStepsForDate = async () => {
            if (isViewingToday) {
                setStepsForSelectedDay(currentSteps);
            } else {
                const history = await getStoredStepsHistory();
                const dateString = getDateString(selectedDate);
                setStepsForSelectedDay(history[dateString] || 0);
            }
        }; 
        if (!isLoading) {
            fetchStepsForDate(); 
        }
    }, [selectedDate, currentSteps, getStoredStepsHistory, isViewingToday, isLoading]);
    useEffect(() => {
    ...
    // --- START OF LOGIC FIX: Replaced the entire updateChallengeStatus function ---
    const updateChallengeStatus = useCallback(async () => {
        ...
        try {
            const [
                storedRemainingDaysStr,
                storedLastParticipationDate,
                storedChallengeDurationStr
            ] = await Promise.all([
            ...
            let loadedDuration = parseInt(storedChallengeDurationStr, 10) || INITIAL_CHALLENGE_DURATION;
            if (!CHALLENGE_DURATIONS.includes(loadedDuration)) {
                loadedDuration = INITIAL_CHALLENGE_DURATION;
            }
            
            let currentRemainingDays = parseInt(storedRemainingDaysStr, 10);
            if (isNaN(currentRemainingDays)) {
                currentRemainingDays = loadedDuration;
            }

            // 1. Is it a new day?
            if (todayString !== storedLastParticipationDate) {
                // 2. If there are days left, decrement
                if (currentRemainingDays > 0) {
                    currentRemainingDays = currentRemainingDays - 1;
                }

                // 3. Now, check if the challenge is completed (or was already completed)
                if (currentRemainingDays <= 0) {
                    // Important: Celebrate the achievement if this is the completion day
                    if (loadedDuration > 0) {
                        ...
                        if (loadedDuration > currentMax) {
                            await AsyncStorage.multiSet([
                                [MAX_COMPLETED_CHALLENGE_DAYS_KEY, String(loadedDuration)],
                                [CELEBRATE_TIER_COMPLETION_KEY, String(loadedDuration)]
                            ]);
                        }
                    }

                    // 4. Find the next challenge and transition to it
                    ...
                    if (nextDurationIndex < CHALLENGE_DURATIONS.length) {
                        ...
                        // Start the new challenge immediately
                        await AsyncStorage.multiSet([
                        ...
                        // Update state to reflect the new challenge
                        setCurrentChallengeDuration(nextChallengeDuration);
                        setRemainingDays(nextChallengeDuration);
                        setLastParticipationDate(todayString);
                    } else {
                        // No more challenges, stay in completed state
                        await AsyncStorage.multiSet([
                    ...
                    // Challenge is not over, just update remaining days and date
                    await AsyncStorage.multiSet([
                        [REMAINING_CHALLENGE_DAYS_KEY, String(currentRemainingDays)],
                        [LAST_PARTICIPATION_DATE_KEY, todayString]
                    ]);
                    setCurrentChallengeDuration(loadedDuration);
                ...
                // Not a new day, just update the state with the stored values
                setCurrentChallengeDuration(loadedDuration);
                setRemainingDays(currentRemainingDays);
            }

        } catch (error) {
            console.error("Challenge update fail:", error);
        }
    }, []);
    // --- END OF LOGIC FIX ---
